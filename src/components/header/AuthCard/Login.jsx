...
const Login = ({ onHideAuthCard }) => {
  const { authState, dispatchSigninWithGoogle } = useAuthContext()
  ...
  useEffect(() => {
    if (email.includes('@') && password.length >= 4 && password.length <= 16) {
      setIsEmailAndPasswordValid(true)
    } else setIsEmailAndPasswordValid(false)
  }, [email, password])
    ...
    if (!isLoggedIn) {
      ...
      doSignInWithGoogle()
        .then(result => {
          ...
          dispatchSigninWithGoogle(user, token)
          // setIsLoggedIn(true)
          onHideAuthCard()
  ...
  return (
    <div className='login'>
      ...
      <Button
        ...
        className={`login__button ${isEmailAndPasswordValid ? '' : ''}`}
        // onClick={}
      >
        {isLoggedIn ? 'Iniciando...' : 'Iniciar'}
      </Button> */}
      {/* <div className='login__registerOption'>
        <span>
          No tienes cuenta? <a onClick={onActivateRegister}>Registrate</a>
        </span>
      </div> */}
      {/* <div className='login__orLineContainer'>
