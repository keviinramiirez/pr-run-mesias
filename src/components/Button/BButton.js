...
const BButton = ({ children, buttonVariant, buttonColor, onClick, style }) => {
  const checkPalette = () => {
  ...
  const isWhiteLabelColor = () => {
    if (isPathSkintech()) return ''
    return buttonVariant === 'outlined' || buttonColor === 'secondary'
      ? 'color__white'
      : ''
  }
        ...
        >
          <Button
            ...
            className={`${isPathEllev8() ? 'blackText' : ''} ${
              isNavbarButton ? 'upDownAnimation' : ''
            }`}
          >
            {children}
          </Button>
